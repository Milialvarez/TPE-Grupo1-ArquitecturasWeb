...
public interface MonopatinRepository extends JpaRepository<Monopatin, Long> {
    ...
    @Query("SELECT m FROM Monopatin m where m.kmRecorridos < :maxKm")
    List<Monopatin> getAllByMaxKm(@PathVariable("maxKm") float maxKm);

    @Query("SELECT m FROM Monopatin m where m.tiempoUso < :maxTiempo")
    List<Monopatin> getAllByMaxTime(@PathVariable("maxTiempo") float maxT);

    @Query("SELECT m FROM Monopatin m where m.tiempoUsoConPausas < :maxTiempo")
    List<Monopatin> getAllByMaxTimeWithPauses(@PathVariable("maxTiempo") float maxT);

    @Query("SELECT m FROM Monopatin m where m.kmRecorridos < :maxKm AND m.tiempoUso < :maxTiempo")
    List<Monopatin> getAllByMaxTimeAndKm(@PathVariable("maxTiempo") float maxT,@PathVariable("maxKm") float maxKm);

    @Query("SELECT m FROM Monopatin m where m.kmRecorridos < :maxKm AND m.tiempoUsoConPausas < :maxTiempo")
